...
> Si usas **Claude.ai Projects** o **Google AI Studio (Gemini)**, sube este archivo junto con `docs/` e `indices/` al Knowledge Base.

@.claude/CLAUDE.md

---

<!-- Reglas completas a continuación para compatibilidad externa -->

## SISTEMA DE HOOKS (solo local CLI)

Si estás en un CLI agéntico, este proyecto tiene **guardrails automáticos** que te bloquean
si intentas violar las reglas. No necesitas que el humano te lo recuerde:

- **Spec Gate**: No puedes codificar sin un track activo (SDD).
...
1. **DESCUBRIR:** Revisa `indices/COMPONENTS.md`, `indices/FACADES.md`, `indices/SERVICES.md`, `indices/DIRECTIVES.md` y `indices/DATABASE.md`. ¿Ya existe algo que resuelva parcial o totalmente el problema? Úsalo.
2. **PLANIFICAR:** Define qué vas a tocar sin violar las reglas de Arquitectura y UI (ver abajo). Crea tu `plan.md` (SDD).
3. **EJECUTAR:** Escribe el código. Prioriza reutilizar. Si hay lógica nueva, escribe tests (TDD).
...
- **Project Knowledge:** Tu contexto te permite leer este documento y toda la carpeta `docs/` y `skills/` al instante. Usa esto a tu favor. No alucines, consulta los archivos.
...
- **Grillas y Layout:** Este proyecto utiliza los Arquetipos de `indices/LAYOUTS.md`. No hagas Grids arbitrarios; utiliza nuestras reglas canónicas.
