...
import { addOutline, closeOutline, trashOutline, timerOutline, flameOutline, checkmarkOutline } from 'ionicons/icons';
...
@Component({
  ...
  imports: [
    ...
    IconComponent, ExerciseSelectorComponent
  ],
  template: `
    <ion-header class="ion-no-border">
      <ion-toolbar style="--background: var(--bg-base);">
        ...
        <ion-title style="color: var(--text-primary); font-weight: 800;">{{ isEditMode() ? 'Editar Rutina' : 'Nueva Rutina' }}</ion-title>
        <ion-buttons slot="end">
          <ion-button (click)="saveRoutine()" [disabled]="!canSave() || facade.isLoading()" color="primary" style="font-weight: 700;">
            @if (facade.isLoading()) {
              Guardando...
            } @else {
        ...
        <div class="exercises-section">
          ...
          @if (exercises().length === 0) {
            <div class="empty-state">
              <app-icon name="dumbbell" [size]="40" style="color: var(--text-muted); opacity: 0.6;"></app-icon>
              <h4>Sin ejercicios en la rutina</h4>
              <p>Presiona "Añadir" para incluir ejercicios a esta plantilla.</p>
            </div>
          } @else {
            <ion-reorder-group [disabled]="false" (ionItemReorder)="handleReorder($any($event))" class="exercises-list">
                ...
                <div class="exercise-card">
                  ...
                  <div class="ex-card-header">
                    ...
                    <div class="ex-header-actions">
                      ...
                      <button class="remove-ex-btn" (click)="removeExercise(ex.id)" title="Eliminar ejercicio">
                        <ion-icon name="close-outline"></ion-icon>
                      </button>
                    </div>
                  </div>
                        ...
                        <div class="set-row">
                          ...
                          <div class="reps-cell">
                            <input 
                              ...
                              (input)="updateSetTargetReps(ex.id, set.id, $any($event.target).value)" />
                          </div>
...
export class RoutineEditorPage implements OnInit {
  ...
  constructor() {
    addIcons({ addOutline, closeOutline, trashOutline, timerOutline, flameOutline, checkmarkOutline });
  }
      ...
      if (routine) {
        ...
        if (routine.routine_exercises && routine.routine_exercises.length > 0) {
          const mapped: DraftExercise[] = routine.routine_exercises.map(rx => {
            ...
            const draftSets: DraftSet[] = (rx.sets && rx.sets.length > 0)
              ? rx.sets.map(s => ({
              ...
              : [
                  { id: crypto.randomUUID(), set_number: 1, set_type: 'normal', target_reps: '8-12' },
    ...
    this.exercises.update(arr => 
      arr.map(e => {
        if (e.id === exerciseId && e.sets.length > 1) {
          const filtered = e.sets.filter(s => s.id !== setId).map((s, idx) => ({
            ...s,
            set_number: idx + 1
          }));
  ...
  getSetTypeLabel(type: RoutineSetType): string {
    switch (type) {
      case 'warmup': return 'Calentamiento (W)';
      case 'dropset': return 'Dropset (D)';
      case 'failure': return 'Al Fallo (F)';
      default: return 'Normal (N)';
    }
  }
