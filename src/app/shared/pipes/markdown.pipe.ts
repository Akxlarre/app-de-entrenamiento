...
export class MarkdownPipe implements PipeTransform {
  ...
  transform(value: string | null | undefined): SafeHtml {
    ...
    // 2. Encabezados (###, ##, #)
    html = html.replace(/^### (.*$)/gim, '<h4 class="text-xs font-semibold text-text-primary uppercase tracking-wider mt-3 mb-1">$1</h4>');
    html = html.replace(/^## (.*$)/gim, '<h3 class="text-sm font-bold text-text-primary mt-3.5 mb-1.5 flex items-center gap-1.5">$1</h3>');
    html = html.replace(/^# (.*$)/gim, '<h2 class="text-base font-bold text-text-primary mt-4 mb-2 pb-1 border-b border-border">$1</h2>');

    // 3. Negritas (**texto** o __texto__)
    html = html.replace(/\*\*(.*?)\*\*/g, '<strong class="font-semibold text-text-primary">$1</strong>');
    html = html.replace(/__(.*?)__/g, '<strong class="font-semibold text-text-primary">$1</strong>');

    // 4. Cursivas (*texto* o _texto_)
    html = html.replace(/\*(.*?)\*/g, '<em class="italic text-text-secondary">$1</em>');
    html = html.replace(/_([^_]+)_/g, '<em class="italic text-text-secondary">$1</em>');

    // 5. Listas de viñetas (- item o * item)
    html = html.replace(/^\s*[-*]\s+(.*$)/gim, '<li class="ml-4 list-disc text-text-secondary text-xs leading-relaxed my-0.5">$1</li>');
      ...
      if (isTableRow) {
        ...
        if (!inTable) {
          ...
          for (const cell of cells) {
            tableHtml += `<th class="py-1.5 px-2.5 font-semibold text-text-primary">${cell}</th>`;
          }
          tableHtml += '</tr></thead><tbody>';
        } else {
          tableHtml += '<tr class="border-b border-border/50 hover:bg-base/30 transition-colors">';
          for (const cell of cells) {
            tableHtml += `<td class="py-1.5 px-2.5 text-text-secondary">${cell}</td>`;
          }
          tableHtml += '</tr>';
        }
      } else {
