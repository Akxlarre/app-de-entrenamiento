...
describe.skip('EmptyStateComponent', () => {
  ...
  it('host aria-label should equal the message', () => {
    expect(fixture.nativeElement.getAttribute('aria-label')).toBe('No hay resultados');
  });
  ...
  it('should NOT render icon container when icon is not provided', () => {
    const iconWrapper = fixture.nativeElement.querySelector("[aria-hidden='true']");
  ...
  it('should render subtitle when provided', () => {
    fixture.componentRef.setInput('subtitle', 'Intenta con otros términos de búsqueda.');
    fixture.detectChanges();
    const paragraphs = fixture.nativeElement.querySelectorAll('p');
    expect(paragraphs.length).toBe(2);
    expect(paragraphs[1].textContent?.trim()).toBe('Intenta con otros términos de búsqueda.');
  });
