...
@Component({
  ...
  template: `
      ...
      <div class="flex items-center justify-between px-5 py-2.5 bg-surface/90 backdrop-blur-md z-10 border-b border-border/40 shrink-0">
        <p class="text-[11px] font-medium text-brand flex items-center gap-1.5 m-0 uppercase tracking-wider">
      ...
      <div #scrollContainer class="flex-1 px-5 py-4 overflow-y-auto flex flex-col gap-4">
        ...
        @if (messages().length === 0) {
          <!-- Empty State -->
          <div class="animate-fade-in-up flex flex-col items-center justify-center my-auto py-8 text-center">
            <div class="w-14 h-14 rounded-2xl bg-brand/10 text-brand flex items-center justify-center mb-3 shadow-inner">
              <app-icon name="sparkles" [size]="26" />
            </div>
            <h3 class="text-base font-semibold text-primary m-0 mb-1">Coach Virtual IA</h3>
            <p class="text-xs text-muted max-w-[80%] mb-6">Tu copiloto durante el entrenamiento. ¿Qué deseas consultar?</p>
            <div class="flex flex-col gap-2 w-full max-w-sm">
              <button type="button" (click)="sendQuickPrompt('¿Cuánto tiempo he descansado desde la última serie?')" class="px-3.5 py-2.5 rounded-2xl bg-base/60 hover:bg-base border border-border/50 text-xs text-primary transition-all shadow-sm text-left flex items-center gap-3 active:scale-[0.99]">
                <span class="text-base">⏱️</span> 
                <span class="font-medium">¿Cuánto tiempo llevo de descanso?</span>
              </button>
              <button type="button" (click)="sendQuickPrompt('¿Qué peso o repeticiones me recomiendas para mi siguiente serie?')" class="px-3.5 py-2.5 rounded-2xl bg-base/60 hover:bg-base border border-border/50 text-xs text-primary transition-all shadow-sm text-left flex items-center gap-3 active:scale-[0.99]">
                <span class="text-base">📈</span> 
                <span class="font-medium">Recomiéndame peso para la siguiente serie</span>
              </button>
              <button type="button" (click)="sendQuickPrompt('Dame un resumen y valoración de mi entrenamiento de hoy.')" class="px-3.5 py-2.5 rounded-2xl bg-base/60 hover:bg-base border border-border/50 text-xs text-primary transition-all shadow-sm text-left flex items-center gap-3 active:scale-[0.99]">
                <span class="text-base">📊</span> 
        ...
        @for (msg of messages(); track msg.id) {
          ...
          >
            ...
            <span class="text-[10px] text-muted mt-1 px-1 font-medium" [class.text-right]="msg.sender === 'user'">
        ...
        @if (isLoading()) {
          <div class="animate-fade-in-up flex flex-col max-w-[75%] self-start gap-1">
            <div class="px-4 py-3 bg-base border border-border/50 rounded-2xl rounded-tl-[4px] shadow-sm flex items-center gap-2.5">
        ...
        @if (messages().length > 0 && !isLoading()) {
          <div class="animate-fade-in-up flex flex-wrap gap-2 pt-1 pb-2">
            <button
              ...
              class="text-[11px] font-medium px-3 py-1.5 rounded-full bg-base border border-border/60 text-muted hover:text-primary hover:border-brand/40 transition-all active:scale-95 shadow-sm"
            >
              ⏱️ Descanso
            </button>
            <button
      ...
      <div class="p-4 bg-surface/95 backdrop-blur-md border-t border-border/40 shrink-0">
        <form (ngSubmit)="send()" class="flex items-center gap-2 bg-base border border-border/60 rounded-full px-3 py-1.5 shadow-sm focus-within:border-brand focus-within:ring-1 focus-within:ring-brand/30 transition-all">
          <input
            type="text"
            [(ngModel)]="inputText"
            name="userPrompt"
