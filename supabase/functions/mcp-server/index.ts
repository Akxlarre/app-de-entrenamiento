...
async function executeTool(authenticatedUserId: string, toolName: string, args: any) {
  ...
  if (toolName === "obtener_mis_entrenamientos_recientes") {
    ...
    const { data, error } = await supabase
      .from("workouts")
      .select("id, routine_id, start_time, end_time, notes, routines(name), workout_reports(energy_level, session_rpe, satisfaction_rating, notes)")
      .eq("user_id", authenticatedUserId)
  ...
  if (toolName === "obtener_series_de_entrenamiento") {
    ...
    const { data, error } = await supabase
      .from("workout_sets")
      .select("id, set_number, set_type, weight, reps, rir, rpe, completed, exercises(name_es, name_en)")
  ...
  if (toolName === "crear_rutina") {
    ...
    return {
      content: [
        {
          type: "text",
          text: JSON.stringify({ message: "Rutina creada exitosamente", routine: newRoutine }, null, 2),
        },
      ],
    };
  }
  ...
  if (toolName === "eliminar_rutina") {
    ...
    return {
      content: [{
        type: "text",
        text: JSON.stringify({ message: "Rutina eliminada exitosamente", deleted_routine_id: args.routine_id }, null, 2),
      }],
    };
  }
  ...
  if (toolName === "buscar_ejercicios") {
    ...
    let query = supabase.from("exercises").select("id, name_es, name_en, category, muscle, equipment");
  ...
  if (toolName === "obtener_entrenamiento_en_curso") {
    ...
    if (workoutErr) {
      if (workoutErr.code === 'PGRST116') {
        return { content: [{ type: "text", text: JSON.stringify({ message: "No tienes ningún entrenamiento en curso actualmente." }, null, 2) }] };
      }
    ...
    const { data: sets, error: setsErr } = await supabase
      .from("workout_sets")
      .select("id, set_number, set_type, weight, reps, rir, rpe, completed, completed_at, exercises(name_es, name_en)")
  ...
  if (toolName === "crear_mesociclo_completo") {
    ...
    if (targetsToInsert.length > 0) {
      const { error: tErr } = await supabase.from('mesocycle_session_targets').insert(targetsToInsert);
      if (tErr) throw new Error("Error en targets: " + tErr.message);
    }

    return { content: [{ type: "text", text: JSON.stringify({ message: "Mesociclo generado exitosamente", id: mesocycleId }) }] };
  }
  ...
  if (toolName === "analizar_volumen_muscular") {
    ...
    if (workoutIds.length === 0) {
       return { content: [{ type: "text", text: JSON.stringify({ message: "No se encontraron entrenamientos en el periodo solicitado." }) }] };
    }
    ...
    return { content: [{ type: "text", text: JSON.stringify({ days_analyzed: daysAgo, volume: volumeByMuscle }, null, 2) }] };
  }
  ...
  if (toolName === "analizar_progresion_ejercicio") {
    ...
    const workoutStats: Record<string, { date: string, maxWeight: number, estimated1RM: number, avgRir: number, countRir: number }> = {};
    ...
    return { content: [{ type: "text", text: JSON.stringify({ exercise_id: args.exercise_id, progression }, null, 2) }] };
  }
    ...
    let query = supabase
      ...
      .select("id, workout_id, category, rating, tags, notes, created_at, exercises(name_es, name_en)")
      .eq("user_id", authenticatedUserId)
      .order("created_at", { ascending: false });
    ...
    tools: [
      {
        ...
        description: "Obtiene las plantillas de rutinas del usuario con sus ejercicios completos ordenados, músculos y categorías.",
      ...
      },
      {
        ...
        description: "Obtiene el detalle (series, reps, peso) de una sesión de entrenamiento específica del usuario.",
        inputSchema: {
          type: "object",
          properties: {
      ...
      {
        name: "crear_rutina",
        description: "Crea una nueva plantilla de rutina de entrenamiento para el usuario autenticado con sus ejercicios.",
        inputSchema: {
          type: "object",
          properties: {
            name: { type: "string", description: "Nombre de la rutina (Ej. 'Empuje A - Hipertrofia')" },
            ...
            exercises: {
              ...
              items: {
                type: "object",
                properties: {
                  exercise_id: { type: "string", description: "UUID del ejercicio obtenido con buscar_ejercicios" },
                  order_index: { type: "number", description: "Posición en la rutina (0, 1, 2...)" },
                },
      ...
      },
      {
        ...
        description: "Busca ejercicios en el catálogo maestro de la app por término o grupo muscular.",
        inputSchema: {
          type: "object",
          properties: {
            search: { type: "string" },
      ...
      },
      {
        ...
        description: "Obtiene el historial de feedback (dolor, técnica, intensidad) reportado por el usuario.",
        inputSchema: {
          type: "object",
          properties: {
      ...
      },
      {
        ...
        description: "Obtiene la sesión de entrenamiento ACTIVA actual del usuario (incluyendo si proviene de una rutina).",
        inputSchema: { type: "object", properties: {} },
      },
      {
      ...
      },
      {
        ...
        description: "Evalúa la progresión histórica (hasta 10 sesiones) en un ejercicio específico calculando 1RM y RIR.",
        inputSchema: {
          type: "object",
          properties: {
      ...
      },
      {
        ...
        description: "Genera un mesociclo completo con sus semanas, sesiones, e inyecta los objetivos (pesos/reps).",
        inputSchema: {
          type: "object",
          properties: {
            ...
            weekly_sessions: {
               type: "array",
               description: "Las sesiones semanales que se repetirán, por ejemplo, [ { day_number: 1, routine_id: '...', targets: [{ exercise_id, set_number, target_weight, target_reps, target_rir }] } ]"
            }
          },
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!user) {
      return new Response(
        JSON.stringify({ error: "No autorizado. Se requiere un JWT válido en el header Authorization." }),
    ...
    try {
      ...
      if (jsonBody && jsonBody.method === "tools/call" && jsonBody.params?.name) {
        const result = await executeTool(user.id, jsonBody.params.name, jsonBody.params.arguments || {});
        return new Response(
          JSON.stringify({
            jsonrpc: "2.0",
